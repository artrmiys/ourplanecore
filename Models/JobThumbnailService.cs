...
public static class JobThumbnailService
{
    ...
    public static string ThumbnailDirectory
    {
        get
        {
            ...
            return Path.Combine(appData, "SmartTakeoffs", "thumbnails");
        }
    }
    ...
    public static bool TryCreateThumbnail(SmartTakeoffsJob job, out string thumbnailPath, out string error)
    {
        thumbnailPath = "";
        error = "";
    ...
    private static PageInfo? FindFirstRenderablePage(string folder)
    {
        ...
        if (SmartTakeoffsJobStore.TryReadPage(folder) is { } page && File.Exists(page.PdfPath))
            return page;
