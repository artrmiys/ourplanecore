...
public static partial class SmartMassingDraftService
{
    ...
    public static SmartMassingDraft BuildDraftFromWallTakeoffs(SmartTakeoffsJob job, double levelSpacingFeet)
    {
        ...
        foreach (WallFloorSource floor in levelSources)
        {
            ...
            if (!TryBuildFootprintFromTakeoffItems(job, sourceItems, sourceKind, out List<SmartMassingPoint> points, out string page, out List<string> sourceIds, out string error))
            {
        ...
        IReadOnlyList<TakeoffItem> allItems)
    {
        ...
        return sources
            .GroupBy(source => Path.GetFullPath(source.FolderPath), StringComparer.OrdinalIgnoreCase)
        ...
        out string error)
    {
        if (string.Equals(sourceKind, "Area", StringComparison.OrdinalIgnoreCase))
            return TryBuildFootprintFromAreaItems(job, items, out footprintPoints, out page, out sourceIds, out error);

        return TryBuildFootprintFromLineItems(job, items, out footprintPoints, out page, out sourceIds, out error);
    }
